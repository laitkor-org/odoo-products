...
class PaymentTransaction(models.Model):
    ...
    def _get_processing_values(self):
        ...
        reference_prefix = self.reference.split('-')[0]
        sale_order = self.env['sale.order'].search([('name', '=', reference_prefix)], limit=1)
        if sale_order:
            delivery_charge = sale_order.delivery_charge
        else:
            delivery_charge = 0.0
        ...
        processing_values = {
            ...
            'amount': self.amount + delivery_charge,
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        self.ensure_one()
        self._ensure_provider_is_not_disabled()
        _logger.info("Amount capture %s and self amount %s",amount_to_capture,self.amount)
