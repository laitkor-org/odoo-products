...
CALCULATE_TARIFF_ENDPOINT = f"{os.getenv('USER_URL')}/api/getDeliveryCharges"
...
class SaleOrder(models.Model):
    ...
    delivery_charge = fields.Monetary(string="Shipping Charge", store=True, compute='_compute_amounts')
    ...
    # Created the fields to store the gift option check and message value
    gift_checked = fields.Boolean(string='Gift option', default=False)
    gift_message = fields.Char(string='Message on the gift',readonly=True)
    qr_code_gift_message = fields.Binary("QR Code")
    shipping_label = fields.Char(string='Shipping Label',readonly=True)
    order_reference_india_post = fields.Char(string='Reference ID for India Post Order')
    total_weight = fields.Float(string='Order Weight')
    ...
    def calculateTotalWeight(self):
        ...
        for line in self.order_line:
            ...
            product_weight = product.weight if product.weight != 0.0 else 2.0
            quantity = line.product_uom_qty
            total_weight += product_weight * quantity

        total_weight = total_weight + 0.45
        self.total_weight = total_weight
        return total_weight

    def getCityName(self, pincode):
        nomi = pgeocode.Nominatim('IN')
        cityinfo = nomi.query_postal_code(pincode)
        if cityinfo is not None and not cityinfo.empty and not cityinfo.isnull().all():
            city_name = cityinfo.county_name
            logging.info("Here is data of pincode: %s", cityinfo)
            if city_name.upper() == "LUCKNOW":
                return "local order"
            else:
                return "not a local order"
        else:
            logging.info("Data is not Found with this pincode: %s", pincode)
            return "invalid pincode"

    def calculateTotalTariff(self, pincode):
        local = self.getCityName(pincode)
        if local == 'local order':
            return 0
        elif local == 'invalid pincode':
            return -1
        total_weight = self.calculateTotalWeight()
        url = CALCULATE_TARIFF_ENDPOINT
        headers = {'Content-Type': 'application/json'}
        body = {'destinationPincode' : pincode, 'total_weight' : total_weight}
        try:
            response = requests.post(url, headers=headers, json=body)
            response.raise_for_status()
            json_response = response.json()
            if json_response['success'] == True:
                total_tariff = json_response['json_response'][0].get('Total Tariff')
            else:
                total_tariff = 0
            return total_tariff
        except requests.exceptions.RequestException as err:
            _logger.info("Failed to call the tariff API : %s", err)
            return 0
    ...
    def _compute_amounts(self):
        ...
        for order in self:
            ...
            # if not delivery_charge_assigned:
            #     self.delivery_charge = self.calculateTotalTariff(zip_code)

        order.amount_total = order.amount_total + order.delivery_charge
    ...
    def _compute_tax_totals(self):
        for order in self:
            ...
            order.tax_totals = self.env['account.tax']._prepare_tax_totals(
                base_lines = [x._convert_to_tax_base_line_dict() for x in order_lines],
                currency = order.currency_id or order.company_id.currency_id, delivery_charge = order.delivery_charge
            )
