...
class AccountMove(models.Model):
    ...
    delivery_charge = fields.Monetary(
        string='Shipping Charge', store=True, readonly=True,
        currency_field='company_currency_id',
    )
    india_post_consignment = fields.Char(string='Consignment Number', store=True, readonly=True)
    qr_code_gift_message = fields.Binary(string='Gift Message QR Code')
    qr_code_gift_message_base = fields.Char(
        string='QR Code Gift Message Base64',
        readonly=True
    )
    ...
    def computeCustomValues(self):
        sale_order = self.env['sale.order'].search([
            ('name', '=', self.invoice_origin)
        ], limit=1)
        if sale_order:
            self.delivery_charge = sale_order.delivery_charge
            if sale_order.order_reference_india_post:
                self.india_post_consignment = sale_order.order_reference_india_post
    ...
    def _compute_tax_totals(self):
        ...
        for move in self:
            if move.is_invoice(include_receipts=True):
                ...
                move.tax_totals = self.env['account.tax']._prepare_tax_totals(**kwargs,delivery_charge = self.delivery_charge)
    ...
    def _compute_amount_total_words(self):
        for move in self:
            move.amount_total_words = move.currency_id.amount_to_text(move.amount_total + move.delivery_charge).replace(',', '')
    ...
    def action_invoice_sent(self):
        ...
        if self.env.is_admin() and not self.env.company.external_report_layout_id and not self.env.context.get('discard_logo_check'):
            return self.addons/account/models/account_move.pyenv['ir.actions.report']._action_configure_external_report_layout(report_action)
    ...
    def set_qr_code_message(self):
        sale_order = self.env['sale.order'].search([
            ('name', '=', self.invoice_origin)
        ], limit=1)
        if sale_order and sale_order.qr_code_gift_message:
            self.qr_code_gift_message = sale_order.qr_code_gift_message
            if isinstance(self.qr_code_gift_message, (bytes, bytearray)):
                encoded_message = self.qr_code_gift_message.decode('utf-8')
                self.qr_code_gift_message_base = encoded_message
            else:
                _logger.info("The received code image isn't formatted in bytes")
        else:
            self.qr_code_gift_message_base = os.getenv('ROOTSRAJA_QR_IMAGE')

    def action_post(self):
        self.set_qr_code_message()
        self.computeCustomValues()
