...
publicWidget.registry.ProductWishlist = publicWidget.Widget.extend(VariantMixin, {
            ...
            if (productId && !self.wishlistProductIDs.includes(productId)) {
                return self.rpc('/shop/wishlist/add', {
                    ...
                    if (productId === currentProductId) {
                        $el.prop("disabled", true).addClass('disabled btn-outline-danger');
                        $el.prop("disabled", true).removeClass('btn-outline-primary');
                    }
                }).catch(function (e) {
                    $el.prop("disabled", true).addClass('btn-outline-primary');
                    $el.prop("disabled", false).removeClass('disabled btn-outline-danger');
    ...
    _updateWishlistView: function () {
        ...
        // }
        // Update the wishlist quantity according to the items present in the wishlist
        if (this.wishlistProductIDs.length === 0) {
            $wishButton.find('.my_wish_quantity').addClass('d-none');
        }
        else {
            $wishButton.find('.my_wish_quantity').removeClass('d-none');
    ...
    _onClickWishRemove: function (ev) {
        ...
        // Adding and removing the empty wishlist alert box according to the items in wishlist
        const $wishEmptyAlert = $('.wish_empty_alert');
        const $wishCheckbox = $('.wish_checkbox');
        if ($wishEmptyAlert.hasClass('d-none') && (this.wishlistProductIDs.length === 0)) {
            $wishEmptyAlert.addClass('d-block');
            $wishEmptyAlert.removeClass('d-none');
            $wishCheckbox.addClass('d-none');
            $wishCheckbox.removeClass('d-block');
        }
