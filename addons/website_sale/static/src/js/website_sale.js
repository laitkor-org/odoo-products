...
export const WebsiteSale = publicWidget.Widget.extend(VariantMixin, cartHandlerMixin, {
    ...
    events: Object.assign({}, VariantMixin.events || {}, {
        ...
        'click .custom_checkbox_field': 'onGiftCheckboxClick',
        'change .giftMessageInput': 'onGiftMessageInputChange',
        'click .calculateChargeOnCheckout': 'calcChargeOnCheckout',
        'click .calculateChargeOnContinueCheckout': 'calcChargeOnContinueCheckout',
        'click .btn_check_delivery': 'checkPincode',
    }),
    ...
    onGiftCheckboxClick: function (ev) {
        let giftMessage = $('.giftMessageInput');
        if (ev.currentTarget.checked == true) {
            giftMessage.removeClass('d-none');
            giftMessage.addClass('d-block');
        }
        else {
            giftMessage.removeClass('d-block');
            giftMessage.addClass('d-none');
        }
    },

    onGiftMessageInputChange: async function (ev) {
        let giftMessage = ev.currentTarget.value;
        try {
            let response = await fetch('/shop/update_gift_message', {
                method: 'POST',
            ...
            if(!data.result.success)
                console.log("Something went wrong. Gift message not sent");
        } catch (error) {
            console.error("Something went wrong. Gift message not sent. Error - ", error);
        }
    },

    calcChargeOnCheckout: async function (ev) {
        let currentButton = ev.currentTarget;
        let pincodeValue = null;
        let checkoutButton = document.getElementById('submitCheckout');
        try {
            let response = await fetch('/shop/calculate_charge', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ pincodeValue })
            });
            let data = await response.json();
            console.log(data);
    ...
    calcChargeOnContinueCheckout: async function (ev) {
        let currentButton = ev.currentTarget;
        let zipInput = document.querySelector('input[name="zip"]');
        let checkoutButton = document.getElementById('submitContinueCheckout');
        let errorContainer = document.getElementById('errorMessages'); 
        let pincodeValue = null;
    
        if (zipInput) {
            pincodeValue = zipInput.value;
        }
        ...
        try {
            ...
            console.log(data);
            if (data.result.success === false) {
                errorContainer.textContent = data.result.message;
            ...
            errorContainer.textContent = 'Please enter a valid pincode';
            errorContainer.style.display = 'inline-block';
            checkoutButton.style.display = "none";
            currentButton.style.display = "inline-block";
        }
    ...
    // function to send the pincode to the controller where the Pincode validation API will be called
    sendPincode: async function (pincode) {
        if(pincode == "")
            return "Empty Input";
        try {
            const response = await fetch('/api/deliveryPincode', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    checkPincode: async function (ev) {
        ev.preventDefault();
        let form = $(ev.currentTarget).closest('form');
        let delivery_pincode = form.find('input[name="custom_input_field_pincode"]').val();
        let helperText = form.find('.helperText');
        let response_data = await this.sendPincode(delivery_pincode);
        let validated_data;
        helperText.removeClass("invisible");
        helperText.addClass("visible");
        if(response_data.result)
        {
            validated_data = response_data.result[0]["Validation Status"];
        }
        if(response_data === "Empty Input")
        {
            helperText.text("Please enter a pincode to check!");
            helperText.addClass("text-danger");
        }
        else if(!response_data.success || !response_data)
        {
            helperText.text("Oops! Something went wrong");
            helperText.addClass("text-danger");
        }
        else
        {
            if(validated_data === "Valid Pincode")
            {
                helperText.text("Yay! This pincode is serviceable");
                helperText.addClass("text-success");
            }
            else
            {
                if(validated_data === "Incorrect Pincode")
                {
                    helperText.text("Sorry! This pincode is not serviceable");
                    helperText.addClass("text-danger");
                }
                else if(validated_data === "Invalid Input")
                {
                    helperText.text("Please enter a valid pincode");
                    helperText.addClass("text-danger");
                }
                else
                {
                    helperText.text(validated_data);
                    helperText.addClass("text-danger");
                }
            }
        }
        setTimeout(function(){
            helperText.addClass("invisible");
            helperText.removeClass("visible");
            helperText.text("Pincode Serviceability");
            helperText.removeClass("text-success");
            helperText.removeClass("text-danger");
        }, 5000);
    },
    ...
    _onClickSubmit: function (ev, forceSubmit) {
        ...
        var $form = $aSubmit.closest('form[name="coupon_code"]');
        if ($form.length > 0 && $aSubmit.is('.a-submit') && !$form.hasClass('disabled')) {
            var $promoInput = $form.find('input[name="promo"]');
...
publicWidget.registry.websiteSaleCarouselProduct = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        // default code that might be required for future references
...
function changeLinkBasedOnDevice() {
    const deviceType = detectDevice();
    const element = document.querySelector('.s_social_media_step-backward');
    ...
    if (element) {
        switch (deviceType) {
            ...
            case "Windows Mobile":
            case "Other Mobile":
                element.href = "https://wa.me/message/RKUAMJATOCC7M1";
                break;
            default:
                element.href = "https://web.whatsapp.com/send?text=https://wa.me/message/RKUAMJATOCC7M1";
                break;
        }
    }
}
changeLinkBasedOnDevice();