...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    def checkout(self, **post):
        ...
        # order_upt = request.env['sale.order'].browse(order_sudo.id)  # Replace `order_id` with the actual order ID
        # _logger.info("Order ID is : %s | Order is : %s",order_sudo.id,str(request.env['sale.order']))

        # # Update the amount_total field
        # order_upt.write({
        #     'amount_total': 12,  # Replace `new_amount_total` with the value you want to set
        # })
    ...
    def generate_qr_code(self, giftMessage):
        qr = qrcode.QRCode(version=1, box_size=4, border=1)
        qr.add_data(giftMessage)
        qr.make(fit=True)
    ...
    @http.route(['/shop/update_gift_message'], type='json', auth="public", website=True, csrf=False)
    def update_gift_message(self):
        data = json.loads(request.httprequest.data)
        order = request.website.sale_get_order()
        giftMessage = data.get('giftMessage')
        sale_order = request.env['sale.order'].sudo().browse(order.id)
        if sale_order:
            qr_code_base64 = self.generate_qr_code(giftMessage)
            sale_order.sudo().write({'gift_message': giftMessage, 'qr_code_gift_message': qr_code_base64})
        else:
            return {'success': False}
        
    @http.route(['/shop/calculate_charge'], type='json', auth="public", website=True, csrf=False)
    def update_shipping_charge(self):
        data = json.loads(request.httprequest.data)
        order = request.website.sale_get_order()
        if data.get('pincodeValue'):
            pincode = data.get('pincodeValue')
        else:
            pincode = order.partner_shipping_id.zip
        if not pincode or len(pincode) != 6:
            return {'success': False, 'message': 'Please enter a valid pincode'}
        shipping_charge = order.calculateTotalTariff(pincode)
        ...
        try:
            ...
            return {'success': True, 'message': 'Shipping charge updated successfully'}
    ...
    def shop_payment(self, **post):
        ...
        if order and not order.only_services and (request.httprequest.method == 'POST' or not order.carrier_id):
            ...
            # code for calculating shipping charge when checkout is clicked (might be required in the future)
            # zip_code = order.partner_id.zip
            # order.delivery_charge = order.calculateTotalTariff(zip_code)
            # order.amount_total = order.amount_total + order.delivery_charge
            # try:
            #     order.sudo().write({
            #         'delivery_charge': order.delivery_charge,
            ...
            #         UPDATE sale_order
            #         SET amount_total = %s, delivery_charge = %s
            #         WHERE id = %s
            #     """, (1200, order.delivery_charge, order.id))
            #     request.env.cr.commit() 
            #     order.invalidate_cache()
            # except Exception as e:
            #     request.env.cr.rollback()
            #     _logger.info(f"An error occurred: {e}")
            # updated_order = request.env['sale.order'].browse(order.id)
            # if updated_order.delivery_charge == order.delivery_charge and updated_order.amount_total == order.amount_total:
            #     logging.info("Data verification successful.")
            # else:
            #     logging.warning("Data verification failed. Changes were not saved as expected.")
