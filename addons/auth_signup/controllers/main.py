...
class AuthSignupHome(Home):
        ...
        if 'error' not in qcontext and request.httprequest.method == 'POST':
            try:
                ...
                if user_sudo and template:
                    ...
                    url = os.getenv('CREATE_USER_WEBHOOK_MAILCHIMP')
                    data = {
                        "email": user_sudo.login
                    }
                    response = requests.post(url, json=data)
                    if response.status_code == 200:
                        _logger.info("User successfully added to the Mailchimp mailing list")
                    else:
                        _logger.info("User could not be added. Error status", response.status_code)
    ...
    def _prepare_signup_values(self, qcontext):
        ...
        password_pattern = re.compile(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z\d])[A-Za-z\d\S]{8,}$')
        if not password_pattern.match(values.get('password')):
            raise UserError(_("Password must contain at least one uppercase letter, one lowercase letter, one digit, and one special character and should be of minimum 8 characters."))
