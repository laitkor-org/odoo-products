...
class CustomerPortal(Controller):
    ...
    def details_form_validate(self, data, partner_creation=False):
        ...
        # Company name validation (only letters and spaces allowed)
        company_name = data.get("company_name", "").strip()
        logging.info("Account Details %s", str(data))
        # Regular expression for company name: only letters (a-z, A-Z) and spaces allowed
        # if not company_name or not re.match(r'^[A-Za-z\s]+$', company_name):
        #     error["company_name"] = 'error'
        #     error_message.append(_("Company Name should contain only letters and spaces."))

        # City name validation (only letters and spaces allowed)
        city = data.get("city", "").strip()
        if not city or not re.match(r'^[A-Za-z\s]+$', city):
            error["city"] = 'error'
            error_message.append(_("City should contain only letters and spaces."))

        # Phone Number Validation (10 digits)
        phone = data.get("phone", "")
        if not phone.isdigit() or len(phone) != 10:
            error["phone"] = 'error'
            error_message.append(_("Phone number must be exactly 10 digits."))

        # Zipcode/Postal Code Validation (6 digits)
        zipcode = data.get("zipcode", "")
        if not zipcode.isdigit() or len(zipcode) != 6:
            error["zipcode"] = 'error'
            error_message.append(_("Postal Code must be exactly 6 digits."))

        # Street Validation
        street = data.get("street", "")
        if not street.strip() or len(street) < 5 or len(street) > 100:
            error["street"] = 'error'
            error_message.append(_("Please enter a street name between 5 and 100 characters."))

        allowed_chars_pattern = re.compile(r'^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d/[\]()\-.,\s]+$')

        # Check if the street contains only allowed characters
        if not allowed_chars_pattern.match(street):
            error["street"] = 'error'
            error_message.append(_("Street address can only contain letters, digits, spaces, commas, slashes, dashes and periods."))

        # Existing validation (if any other validations are needed)
        if "name" in data and not data.get("name"):
            error["name"] = 'error'
            error_message.append(_("Name is required."))
    ...
    def _get_http_headers(self, model, report_type, report, download):
        ...
        return headers
    @http.route('/portal/pincode_data', type='json', auth='public', methods=['POST'], csrf=False)
    def get_pincode_data(self):
        ...
        csv_file_path = os.path.join(basepath, 'pincode.csv')
        ...
        if os.path.exists(csv_file_path):
            ...
            try:
                with open(csv_file_path, mode='r', encoding='utf-8') as file:
                  ...
                  for row in reader:
                      row_pincode = row['Pincode'].strip().replace('"', '')
                      zipcode_cleaned = zipcode.strip().replace('"', '').strip()
                    
                      if row_pincode == zipcode_cleaned:
                            logging.info("pincode getting from csv file %s", row_pincode)
                            data.append({
                                'Pincode': row_pincode,
                                'City': row['District'].strip().replace('"', '').title(),
                                'State': row['StateName'].strip().replace('"', '').title(),
                            })

                            break
                if not data:
                  return json.dumps({'jsonrpc': '2.0', 'result': {'error': 'Pincode not found'}})
